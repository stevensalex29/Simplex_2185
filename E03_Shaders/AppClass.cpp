...
{
	m_uShaderProgramID = LoadShaders("Shaders//BasicColor.vs", "Shaders//Complementary.fs");
...
void AppClass::ProcessKeyboard(sf::Event a_event)
{
	...
	else if (sf::Keyboard::isKeyPressed(sf::Keyboard::C)) //Toggle complementary color flag
...
void AppClass::Display(void)
{
	...
	GLuint comp = glGetUniformLocation(m_uShaderProgramID, "Complimentary");
	if (m_complementary) {
		glUniform1i(comp, true); //Set shader variable as true to toggle complimentary 
		glUniform3f(SolidColor, m_v3Color.r, m_v3Color.g, m_v3Color.b);
	}
	else {
		glUniform1i(comp, false);
		glUniform3f(SolidColor, m_v3Color.r, m_v3Color.g, m_v3Color.b);
	}
